...
vector<Vector2> eye_tex_offset_list{ Vector2(0.0f, 0.0f) , Vector2(0.0f, -0.25f), 
														Vector2(0.0f, -0.5f), Vector2(0.0f, -0.75f), 
														Vector2(0.5f, 0.0f), Vector2(0.5f, -0.25f), Vector2(0.5f, -0.5f) };

// Render function for display rendering
void RenderScene(int per_vertex_or_per_pixel) {	
	Vector3 modelPos = models[cur_idx].position;
	//find texture start positions
	...
	Matrix4 MV = view_matrix * model_matrix;
	GLfloat mv[16];
	setGLMatrix(mv, MV);
	glUniformMatrix4fv(iLocmv, 1, GL_FALSE, mv);

	glUniform1i(iLocperpix, per_vertex_or_per_pixel);
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case LightEdit:
		if (light_idx==0)
		{
			Ld1 += Vector3(0.05f, 0.05f, 0.05f)*(float)yoffset;
		}
		else if (light_idx == 1)
		{
			Ld2 += Vector3(0.05f, 0.05f, 0.05f)*(float)yoffset;
		}
		else
		{
			if (spotcutoff <= 180 and spotcutoff >= 0)
			{
				spotcutoff += 5 * (float)yoffset;
				//cout << spotcutoff << endl;
			}
		}
		break;
	case ShininessEdit:
		shininess += 1 * (float)yoffset;
		break;
	default:
		break;
	}
...
void normalization(tinyobj::attrib_t* attrib, vector<GLfloat>& vertices, vector<GLfloat>& colors, vector<GLfloat>& normals, vector<GLfloat>& textureCoords, vector<int>& material_id, tinyobj::shape_t* shape)
{
	...
	for (size_t f = 0; f < shape->mesh.num_face_vertices.size(); f++) {
		...
		for (size_t v = 0; v < fv; v++) {
			...
			//cout << "tex coordinate:"<<attrib->texcoords[2 * idx.texcoord_index + 0]<<","<< attrib->texcoords[2 * idx.texcoord_index + 1]<<endl;
...
void setUniformVariables()
{
	...
	iLocLd1 = glGetUniformLocation(program, "Ld1");
	iLocLd2 = glGetUniformLocation(program, "Ld2");
	iLocLd3 = glGetUniformLocation(program, "Ld3");
	iLocLp1 = glGetUniformLocation(program, "lightPos1");
	iLocLp2 = glGetUniformLocation(program, "lightPos2");
	iLocLp3 = glGetUniformLocation(program, "lightPos3");
	iLocspotcutoff = glGetUniformLocation(program, "spotCutoff");
	iLocperpix = glGetUniformLocation(program, "perpix");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "110065521 HW3", NULL, NULL);
    if (window == NULL)
    {
